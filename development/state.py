...
# Ensure indexes
db.dev_workflows.create_index("brand_id")
db.dev_workflow_nodes.create_index([("workflow_id", 1), ("stage_key", 1)])
db.dev_workflow_transitions.create_index("workflow_id")
db.dev_workflow_states.create_index("workflow_id", unique=True)
db.dev_agent_states.create_index([("workflow_id", 1), ("agent_name", 1)])
db.dev_user_sessions.create_index([("workflow_id", 1), ("user_id", 1)])
...
class DevWorkflowState:
    ...
    def _load(self) -> dict:
        if self._doc is None:
            self._doc = db.dev_workflows.find_one({"_id": ObjectId(self.workflow_id)})
        return self._doc
    ...
    def get_all_stage_outputs(self) -> Dict[str, Any]:
        nodes = db.dev_workflow_nodes.find({"workflow_id": self.workflow_id})
    ...
    def transition_to(self, next_stage: str, trigger: str = "auto", actor_id: str = None, metadata: dict = None):
        ...
        db.dev_workflow_transitions.insert_one({
            "workflow_id": self.workflow_id,
        ...
        db.dev_workflow_nodes.update_one(
            ...
            {"$set": update_fields,
             "$setOnInsert": {
                 "workflow_id": self.workflow_id,
                 "stage_key": stage_key,
                 "stage_index": get_dev_stage_index(stage_key),
    ...
    def get_transition_history(self) -> List[dict]:
        transitions = list(db.dev_workflow_transitions.find(
            {"workflow_id": self.workflow_id}
        ).sort("timestamp", 1))
...
class DevWorkflowStateStore:
    ...
    def save(workflow_id: str, state_data: dict):
        now = datetime.utcnow()
        db.dev_workflow_states.update_one(
    ...
    def load(workflow_id: str) -> Optional[dict]:
        doc = db.dev_workflow_states.find_one({"workflow_id": workflow_id})
        if doc:
            doc["_id"] = str(doc["_id"])
        return doc
...
class DevAgentStateStore:
    ...
    def save(workflow_id: str, agent_name: str, state_data: dict):
        now = datetime.utcnow()
        db.dev_agent_states.update_one(
    ...
    def load(workflow_id: str, agent_name: str) -> Optional[dict]:
        doc = db.dev_agent_states.find_one({"workflow_id": workflow_id, "agent_name": agent_name})
        if doc:
            doc["_id"] = str(doc["_id"])
        return doc
    ...
    def list_agents(workflow_id: str) -> List[dict]:
        agents = list(db.dev_agent_states.find({"workflow_id": workflow_id}))
        for a in agents:
            a["_id"] = str(a["_id"])
        return agents
    ...
    def clear(workflow_id: str, agent_name: str):
        db.dev_agent_states.delete_one({"workflow_id": workflow_id, "agent_name": agent_name})
...
class DevUserSession:
    ...
    def get_or_create(workflow_id: str, user_id: str) -> dict:
        now = datetime.utcnow()
        session = db.dev_user_sessions.find_one({"workflow_id": workflow_id, "user_id": user_id})
        if not session:
            doc = {
                "workflow_id": workflow_id,
                "user_id": user_id,
            ...
            result = db.dev_user_sessions.insert_one(doc)
            session = db.dev_user_sessions.find_one({"_id": result.inserted_id})
    ...
    def update_chat_context(workflow_id: str, user_id: str, messages: List[dict]):
        MAX_CONTEXT = 50
        now = datetime.utcnow()
        db.dev_user_sessions.update_one(
    ...
    def get_active_sessions(workflow_id: str) -> List[dict]:
        sessions = list(db.dev_user_sessions.find({"workflow_id": workflow_id}))
        for s in sessions:
            s["_id"] = str(s["_id"])
        return sessions
