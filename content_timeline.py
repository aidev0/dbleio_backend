#!/usr/bin/env python3
"""
Content Workflow Timeline Entries API.
Chat messages, status updates, and activity feed for content workflows.
"""
...
router = APIRouter(prefix="/api/content/workflows/{workflow_id}/timeline", tags=["content-timeline"])
...
def _resolve_author_name(author_id: str) -> str:
    """Resolve a display name from a workos_user_id."""
    if not author_id:
        return "Unknown"
    user = db.users.find_one({"workos_user_id": author_id})
    if user:
        first = user.get("first_name", "")
        last = user.get("last_name", "")
        if first or last:
            return f"{first} {last}".strip()
        return user.get("email", author_id)
    return author_id
...
def _entry_helper(doc) -> dict:
    ...
    return {
        ...
        "author_name": doc.get("author_name") or _resolve_author_name(doc.get("author_id")),
        ...
        "todos": doc.get("todos"),
        "approval_data": doc.get("approval_data"),
        "status_data": doc.get("status_data"),
...
async def list_timeline_entries(workflow_id: str, request: Request, visibility: Optional[str] = None):
    """List timeline entries for a workflow."""
    try:
        ...
        entries = list(db.content_timeline_entries.find(query).sort("created_at", 1))
...
async def create_timeline_entry(workflow_id: str, body: TimelineEntryCreate, request: Request):
    ...
    try:
        from auth import require_user_id
        workos_user_id = require_user_id(request)
        workflow = _verify_access(workflow_id, workos_user_id)
        ...
        result = db.content_timeline_entries.insert_one(doc)
        entry = db.content_timeline_entries.find_one({"_id": result.inserted_id})
...
async def update_timeline_entry(workflow_id: str, entry_id: str, body: TimelineEntryUpdate, request: Request):
    ...
    try:
        ...
        entry = db.content_timeline_entries.find_one({"_id": ObjectId(entry_id), "workflow_id": workflow_id})
        if not entry:
        ...
        db.content_timeline_entries.update_one({"_id": ObjectId(entry_id)}, {"$set": update_dict})

        updated = db.content_timeline_entries.find_one({"_id": ObjectId(entry_id)})
...
async def delete_timeline_entry(workflow_id: str, entry_id: str, request: Request):
    ...
    try:
        ...
        db.content_timeline_entries.update_one(
